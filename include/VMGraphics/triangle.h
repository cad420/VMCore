...
public:
	Triangle( std::shared_ptr<TriangleMesh> mesh, int indexOffset, std::shared_ptr<Material> m = nullptr ) :
	  Shape( m )
	{
	...
	Bound3f bound() const override
	{
		return Bound3f( m_sharedTriangles->m_vertices[ m_vertexIndices[ 0 ] ],
						m_sharedTriangles->m_vertices[ m_vertexIndices[ 1 ] ] )
		  .UnionWith(
			m_sharedTriangles->m_vertices[ m_vertexIndices[ 2 ] ] );
	}
	Float area() const override
	{
		const auto v1 = m_sharedTriangles->m_vertices[ m_vertexIndices[ 1 ] ] - m_sharedTriangles->m_vertices[ m_vertexIndices[ 0 ] ];
		const auto v2 = m_sharedTriangles->m_vertices[ m_vertexIndices[ 2 ] ] - m_sharedTriangles->m_vertices[ m_vertexIndices[ 0 ] ];
