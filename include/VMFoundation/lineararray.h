...
public:
	...
	LocalBuffer( const LocalBuffer &array ) :
...
public:
	Linear1DArray() :
	  size( 0 ), data( nullptr ), own( true )
	{
	}
	Linear1DArray( size_type x, T *data, bool own ) :
	  size( x ), data( data ), own( own )
	{
	}

	Linear1DArray( size_type x, const T *data ) :
	  Linear1DArray( x, nullptr, true )
	{
	...
	Linear1DArray( const Linear1DArray &array ) :
...
public:
	Linear2DArray() :
	  size( 0, 0 ), m_data( nullptr ), m_own( true )
	{
	}
	...
	Linear2DArray( size_type x,
				   ...
				   T *data,
				   bool own ) :
	  size( x, y ), m_data( data ), m_own( own )
	{
	}

	Linear2DArray( size_type x, size_type y, const T *data ) :
	  Linear2DArray( x, y, nullptr, true )
	{
	...
	Linear2DArray( const Size2 &size, const T *data ) :
	  Linear2DArray( size.x, size.y, data )
	{
	}

	Linear2DArray( const Linear2DArray &array ) :
...
public:
	Linear3DArray() :
	  size( 0, 0, 0 ), m_data( nullptr ), xyPlaneSize( 0 ), own( true )
	{
	}
	...
	Linear3DArray( size_type x,
				   ...
				   T *data,
				   bool own ) :
	  size( x, y, z ), m_data( data ), own( own ), xyPlaneSize( x * y )
	{
	}

	Linear3DArray( size_type x, size_type y, size_type z, const T *data ) :
	  Linear3DArray( x, y, z, nullptr, true )
	{
		const auto t = x * y * z;
	...
	Linear3DArray( const Size3 &sze,
				   T *data ) :
	  Linear3DArray( sze.x, sze.y, sze.z, data )
	{
	}

	Linear3DArray( const Linear3DArray &array ) :
